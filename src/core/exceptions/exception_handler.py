...
class ExceptionHandler(Singleton):
    ...
    async def handle(request: Request, exc: Exception) -> Response:
        ...
        if isinstance(exc, S3ProxyServiceException):
            return JSONResponse(status_code=500, content={"message": exc.get_message()})
        ...
        unknown_error_message = ExceptionHandler.translation_manager.translate("errors.generic.unknown_error")
